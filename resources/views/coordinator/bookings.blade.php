...
<div class="w-full min-h-screen px-8 py-6 space-y-6">
    ...
    <!-- SEARCH -->
<form method="GET" action="{{ route('coordinator.bookings') }}" class="flex items-center gap-3 w-full">
...
<tr class="hover:bg-[#A1BC98]/20 transition">
    <td class="py-4 px-6 font-medium text-[#3E3F29]">{{ $booking->name }}</td>
    <td class="py-4 px-6 text-gray-700">{{ $booking->event_requested }}</td>
    <td class="py-4 px-6">
        <span class="inline-block px-3 py-1 text-xs rounded-full
                     {{ $booking->status == 'Pending' ? 'bg-[#A1BC98] text-[#3E3F29]' : 'bg-green-200 text-green-800' }}">
            {{ $booking->status }}
        </span>
    </td>
    <td class="py-4 px-6 text-center">
        <a href="{{ route('coordinator.bookings.show', $booking->id) }}"
           class="inline-block px-4 py-2 text-xs rounded-lg bg-[#778873] text-white hover:bg-[#3E3F29] transition">
            View
        </a>
    </td>
</tr>
